    ...
    return (
        <div key={id} className="sub-slider-outer">
            <div className="sub-slider">
                ...
                {
                    isAdmin ? <i></i> : toggle.includes(id) ? <i onClick={() => { removeToCart(id, name, prize, 1) }} className="cart-icon fas fa-minus-circle"></i> : <i onClick={() => { addToCart(id, name, prize, 1) }} className="cart-icon fas fa-cart-plus"></i>
                }
            </div>
